...
def fill_frames(actual_labels, supposed_no_labels):
    ...
    elif no_labels < supposed_no_labels:  # here, less frames in df than in video. spread data
        np.random.seed(4)
        positions = sorted(
            np.random.choice(supposed_no_labels, no_labels, replace=False))  # which positions do we want to fill?
    else:  # here, more or equal frames than in video. cut off at end
